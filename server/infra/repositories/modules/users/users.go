...
func (u *userRepository) GetUser(id string) (*types.UsersSingleResponse, error) {
	...
	user := &types.UsersSingleResponse{}
	var rolesID []uint8
...
func (u *userRepository) GetUsers(page types.Page) ([]types.UsersSingleResponse, error) {
	rows, err := db.DB.Query(`
	SELECT u.id, u.account, u.nickname, GROUP_CONCAT(ur.role_id), u.is_admin, u.avatar,u.status,u.department_id, u.create_time, u.update_time
	FROM users u
	LEFT JOIN users_roles ur ON u.id = ur.user_id
...
`, page.Offset, page.Limit)
	...
	users := make([]types.UsersSingleResponse, 0)
	for rows.Next() {
		user := &types.UsersSingleResponse{}
		var rolesID []uint8
		err := rows.Scan(&user.ID, &user.Account, &user.Nickname, &rolesID, &user.IsAdmin, &user.Avatar, &user.Status, &user.DepartmentID, &user.CreateTime, &user.UpdateTime)
...
func (u *userRepository) CreateUser(user *types.CreateSingleUserRequest) int64 {
	id := utils.GenID()
...
func (u *userRepository) FindUserByParams(params *types.QueryUsersParams) (*types.HasTotalResponseData, error) {
	...
	return &types.HasTotalResponseData{
		Total: total,
		List:  users,
	}, nil
}

func (u *userRepository) FindUserByParamsAndOutRoleID(roleId string, params *types.QueryUsersParams) (*types.HasTotalResponseData, error) {
	count := `
...
func (u *userRepository) FindUserByAccount(account string) (*types.SingleUserResponseHasPassword, bool) {
	if account == "" {
		return nil, false
	}
	user := &types.SingleUserResponseHasPassword{}
...
func (u *userRepository) FindUserById(id string) (*types.UsersSingleResponse, bool) {
...
func (u *userRepository) UpdateUser(params *types.UpdateUserRequest, id string) error {
...
func (u *userRepository) GetUserByRoleID(roleID string, params types.Page) (*types.HasTotalResponseData, error) {
	var users []*types.SingleUserByRoleIDResponse
	...
	for rows.Next() {
		var user types.SingleUserByRoleIDResponse
		var rolesID []uint8
		err := rows.Scan(
			&user.ID,
			&user.Account,
