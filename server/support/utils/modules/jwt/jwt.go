...
func (j *Jsonwebtoken) GenerateTokenUsingHs256(jwtPayload *types.JWTPayload) (string, error) {
	claim := types.JWTPayload{
		ID:           jwtPayload.ID,
...
func (j *Jsonwebtoken) ParseTokenHs256(tokenStr string) (*types.JWTPayload, error) {
	token, err := jwt.ParseWithClaims(tokenStr, &types.JWTPayload{}, func(token *jwt.Token) (interface{}, error) {
	...
	claims, ok := token.Claims.(*types.JWTPayload)
	if !ok {
